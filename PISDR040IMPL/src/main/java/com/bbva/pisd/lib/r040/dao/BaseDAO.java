...
public class BaseDAO {
    ...
    public Object executeQuery(Operation operation) {
        ...
        try {
            ...
            throw new BusinessException(DatabaseParticipantErrors.QUERY_EMPTY_RESULT.getAdviceCode(), false, ex.getMessage());
        } catch(DuplicateKeyException ex) {
            LOGGER.info("[BaseDAO] - DuplicateKeyException {}", operation.getNameProp());
            throw new BusinessException(DatabaseParticipantErrors.ERROR_DUPLICATE_KEY.getAdviceCode(), true, ex.getMessage());
        } catch (TimeoutException ae){
            LOGGER.info("[BaseDAO] - TimeoutException {}", operation.getNameProp());
            throw new BusinessException(DatabaseParticipantErrors.ERROR_TIME_OUT.getAdviceCode(), false, ae.getMessage());
        }catch (IncorrectResultSizeException ae){
            LOGGER.info("[BaseDAO] - TimeoutException {}", operation.getNameProp());
            throw new BusinessException(DatabaseParticipantErrors.ERROR_INCORRECT_RESULT.getAdviceCode(), false, ae.getMessage());
        }
